...
router.post('/:id/recibir', async (req, res) => {
  ...
  try {
    ...
    // que recalculó productos.stock_actual = SUM(lotes aprobados). El lote
    // entra en 'cuarentena', así que el trigger NO modifica stock_actual hasta
    // que QC lo apruebe. Por tanto antes==despues en el stock_move de recepción
    // (la entrada real al disponible se registra al pasar el lote a 'aprobado'
    // desde lotes.routes.ts).
